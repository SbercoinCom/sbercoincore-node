...
var bitcore = require('sbercoincore-lib');
var zmq = require('zmq');
var async = require('async');
var LRU = require('lru-cache');
var SbercoinRPC = require('sbercoind-rpc');
...
/**
 * Provides a friendly event driven API to sbercoind in Node.js. Manages starting and
 * stopping sbercoind as a child process for application support, as well as connecting
 * to multiple sbercoind processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Sbercoin(options) {
  if (!(this instanceof Sbercoin)) {
    return new Sbercoin(options);
  }
...
util.inherits(Sbercoin, Service);

Sbercoin.dependencies = [];

Sbercoin.DEFAULT_MAX_TXIDS = 1000;
Sbercoin.DEFAULT_MAX_HISTORY = 50;
Sbercoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Sbercoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Sbercoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Sbercoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
Sbercoin.DEFAULT_SPAWN_STOP_TIME = 10000;
Sbercoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
Sbercoin.DEFAULT_REINDEX_INTERVAL = 10000;
Sbercoin.DEFAULT_START_RETRY_INTERVAL = 5000;
Sbercoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Sbercoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Sbercoin.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Sbercoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Sbercoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Sbercoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Sbercoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Sbercoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Sbercoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Sbercoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Sbercoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Sbercoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Sbercoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Sbercoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Sbercoin.prototype._initCaches = function() {
...
Sbercoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Sbercoin.prototype.getAPIMethods = function() {
  var methods = [
...
Sbercoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'sbercoind/' + name, 'total:', this.subscriptions[name].length);
};

Sbercoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'sbercoind/' + name, 'total:', this.subscriptions[name].length);
};


Sbercoin.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
Sbercoin.prototype.unsubscribeBalance = function(emitter, addresses) {
...
Sbercoin.prototype.unsubscribeBalanceAll = function(emitter) {
...
Sbercoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'sbercoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Sbercoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'sbercoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Sbercoin.prototype.unsubscribeAddressAll = function(emitter) {
...
Sbercoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Sbercoin.DEFAULT_CONFIG_SETTINGS;
...
Sbercoin.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Sbercoin.prototype._expandRelativeDatadir = function() {
...
Sbercoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in sbercoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in sbercoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in sbercoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './sbercoin.conf');

  log.info('Using sbercoin config file:', configPath);
...
Sbercoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of sbercoincore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of sbercoincore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of sbercoincore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to sbercoind from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from sbercoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from sbercoind. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that sbercoind is undergoing a reindex. ' +
             ...
             'of sbercoincore-node services will start.');
    node._reindex = true;
  }
};

Sbercoin.prototype._resetCaches = function() {
...
Sbercoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Sbercoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Sbercoin.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Sbercoin Daemon Ready');
          callback();
        });
      });
...
Sbercoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8332
  };
...
Sbercoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/sbercoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/sbercoin.conf';
    }
  }
  return networkPath;
};

Sbercoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Sbercoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('sbercoind/hashblock', message.toString('hex'));
    }
  }

};

Sbercoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Sbercoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Sbercoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Sbercoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Sbercoin.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Sbercoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
Sbercoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('sbercoind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Sbercoin.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('sbercoind/rawtransaction', message.toString('hex'));
    }
...
Sbercoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Sbercoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Sbercoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Sbercoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Sbercoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Sbercoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Sbercoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Sbercoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Sbercoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/sbercoind.pid';
...
Sbercoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting sbercoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Sbercoin process unexpectedly exited with code:', code);
        log.warn('Restarting sbercoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Sbercoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn sbercoind.'));
      }
...
Sbercoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to sbercoind.'));
    }
 ...
 */
Sbercoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Sbercoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
Sbercoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, sbercoincore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Sbercoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Sbercoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Sbercoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Sbercoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in sbercoincore is most recent to less recent
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Sbercoin.prototype._getConfirmationsDetail = function(transaction) {
...
Sbercoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Sbercoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Sbercoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A sbercoincoin transaction id
 * @param {Function} callback
 */
Sbercoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Sbercoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Sbercoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Sbercoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Sbercoin.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Sbercoincore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Sbercoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getInfo = function(callback) {
...
Sbercoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('sbercoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('sbercoind process did not exit'));
      }
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Sbercoin.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Sbercoin.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Sbercoin.prototype.getStakingInfo = function(callback) {
    var self = this,
