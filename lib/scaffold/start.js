...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for sbercoind is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and sbercoincore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed sbercoincore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './sbercoincore-node.json');
  ...
  node.on('ready', function() {
    log.info('Sbercoincore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in sbercoincore-node services directory
    ...
    if (servicePackage.sbercoincoreNode) {
      serviceModule = service.name + '/' + servicePackage.sbercoincoreNode;
    }
