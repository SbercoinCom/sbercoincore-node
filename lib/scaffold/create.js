...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'sbercoincore-lib': 'https://github.com/SbercoinCom/sbercoincore-lib.git#master',
    'sbercoincore-node': 'https://github.com/SbercoinCom/sbercoincore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/sbercoincore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the sbercoincore-node directory and configuration
