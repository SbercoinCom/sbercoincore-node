...
describe('Node Functionality', function() {
    ...
    rimraf(datadir + '/regtest', function(err) {
      ...
      node.start(function(err) {
        ...
        var syncedHandler = function() {
          if (node.services.sbercoind.height === 150) {
            node.services.sbercoind.removeListener('synced', syncedHandler);
            done();
          }
        };

        node.services.sbercoind.on('synced', syncedHandler);
  ...
  describe('Bus Functionality', function() {
    it('subscribes and unsubscribes to an event on the bus', function(done) {
      ...
      var blockExpected;
      var blockReceived;
      bus.subscribe('sbercoind/hashblock');
      bus.on('sbercoind/hashblock', function(data) {
        bus.unsubscribe('sbercoind/hashblock');
        if (blockExpected) {
  ...
  describe('Address Functionality', function() {
    ...
    before(function(done) {
      ...
      var startHeight = node.services.sbercoind.height;
      node.services.sbercoind.on('tip', function(height) {
    ...
    describe('Mempool Index', function() {
      ...
      it('will update the mempool index after new tx', function(done) {
        ...
        node.services.sbercoind.sendTransaction(tx.serialize(), function(err, hash) {
