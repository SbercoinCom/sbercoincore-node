...
var SbercoinService = require('../../lib/services/sbercoind');
...
describe('#start', function() {
  ...
  describe('will dynamically create a node from a configuration', function() {
    ...
    it('require each sbercoincore-node service with default config', function(done) {
      var node;
      var TestNode = function(options) {
        options.services[0].should.deep.equal({
          name: 'sbercoind',
          module: SbercoinService,
          config: {
            spawn: {
              datadir: './data'
            }
    ...
    it('require each sbercoincore-node service with explicit config', function(done) {
      var node;
      var TestNode = function(options) {
        options.services[0].should.deep.equal({
          name: 'sbercoind',
