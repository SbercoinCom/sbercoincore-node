...
var sbercoincore = require('sbercoincore-lib');
var _ = sbercoincore.deps._;
...
var Transaction = sbercoincore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/sbercoin.conf')));
var SbercoinService = proxyquire('../../lib/services/sbercoind', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultSbercoinConf = fs.readFileSync(path.resolve(__dirname, '../data/default.sbercoin.conf'), 'utf8');

describe('Sbercoin Service', function() {
	...
	var baseConfig = {
		node: {
			network: sbercoincore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var sbercoind = new SbercoinService(baseConfig);
			should.exist(sbercoind);
		});
		it('will create an instance without `new`', function() {
			var sbercoind = SbercoinService(baseConfig);
			should.exist(sbercoind);
		});
		it('will init caches', function() {
			var sbercoind = new SbercoinService(baseConfig);

			should.exist(sbercoind.utxosCache);
			should.exist(sbercoind.txidsCache);
			should.exist(sbercoind.balanceCache);
			should.exist(sbercoind.summaryCache);
			should.exist(sbercoind.transactionDetailedCache);
			should.exist(sbercoind.accountInfo);

			should.exist(sbercoind.transactionCache);
			should.exist(sbercoind.rawTransactionCache);
			should.exist(sbercoind.rawJsonTransactionCache);
			should.exist(sbercoind.blockCache);
			should.exist(sbercoind.blockJsonCache);
			should.exist(sbercoind.blockSubsidyCache);
			should.exist(sbercoind.rawBlockCache);
			should.exist(sbercoind.blockHeaderCache);
			should.exist(sbercoind.zmqKnownTransactions);
			should.exist(sbercoind.zmqKnownBlocks);
			should.exist(sbercoind.lastTip);
			should.exist(sbercoind.lastTipTimeout);
			should.equal(sbercoind.dgpInfoCache, null, 'should be null');
			should.equal(sbercoind.miningInfoCache, null, 'should be null');
			should.equal(sbercoind.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(sbercoind.maxTxids, 1000);
			should.equal(sbercoind.maxTransactionHistory, 50);
			should.equal(sbercoind.maxAddressesQuery, 10000);
			should.equal(sbercoind.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(sbercoind.spawnRestartTime, 5000);
			should.equal(sbercoind.spawnStopTime, 10000);

			// try all interval
			should.equal(sbercoind.tryAllInterval, 1000);
			should.equal(sbercoind.startRetryInterval, 5000);

			// rpc limits
			should.equal(sbercoind.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(sbercoind.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.nodes.should.deep.equal([]);
			sbercoind.nodesIndex.should.equal(0);
			sbercoind.nodes.push({ client: sinon.stub() });
			should.exist(sbercoind.client);
		});
		it('will set subscriptions', function() {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind._initDefaults({ transactionConcurrency: 10 });
			sbercoind.transactionConcurrency.should.equal(10);
			sbercoind._initDefaults({});
			sbercoind.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			SbercoinService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var methods = sbercoind.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var events = sbercoind.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('sbercoind/rawtransaction');
			events[0].scope.should.equal(sbercoind);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('sbercoind/hashblock');
			events[1].scope.should.equal(sbercoind);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('sbercoind/addresstxid');
			events[2].scope.should.equal(sbercoind);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('sbercoind/addressbalance');
			events[3].scope.should.equal(sbercoind);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.subscribe = sinon.stub();
			sbercoind.unsubscribe = sinon.stub();
			var events = sbercoind.getPublishEvents();

			events[0].subscribe('test');
			sbercoind.subscribe.args[0][0].should.equal('rawtransaction');
			sbercoind.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			sbercoind.unsubscribe.args[0][0].should.equal('rawtransaction');
			sbercoind.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			sbercoind.subscribe.args[1][0].should.equal('hashblock');
			sbercoind.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			sbercoind.unsubscribe.args[1][0].should.equal('hashblock');
			sbercoind.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var emitter = {};
			sbercoind.subscribe('hashblock', emitter);
			sbercoind.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			sbercoind.subscribe('rawtransaction', emitter2);
			sbercoind.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			sbercoind.subscribe('hashblock', emitter1);
			sbercoind.subscribe('hashblock', emitter2);
			sbercoind.subscribe('hashblock', emitter3);
			sbercoind.subscribe('hashblock', emitter4);
			sbercoind.subscribe('hashblock', emitter5);
			sbercoind.subscriptions.hashblock.length.should.equal(5);

			sbercoind.unsubscribe('hashblock', emitter3);
			sbercoind.subscriptions.hashblock.length.should.equal(4);
			sbercoind.subscriptions.hashblock[0].should.equal(emitter1);
			sbercoind.subscriptions.hashblock[1].should.equal(emitter2);
			sbercoind.subscriptions.hashblock[2].should.equal(emitter4);
			sbercoind.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			sbercoind.subscriptions.hashblock = [emitter1];
			sbercoind.unsubscribe('hashblock', emitter3);
			sbercoind.subscriptions.hashblock.length.should.equal(1);
			sbercoind.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			sbercoind.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(sbercoind.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var emitter = new EventEmitter();
			sbercoind.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			sbercoind.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			sbercoind.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			sbercoind.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			sbercoind.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			sbercoind.unsubscribeAddress(emitter1);
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			sbercoind.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			sbercoind.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			sbercoind.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			sbercoind.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			sbercoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(sbercoind, 'unsubscribeAddressAll');
			sbercoind.unsubscribeAddress(emitter1);
			sbercoind.unsubscribeAddressAll.callCount.should.equal(1);
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			sbercoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			sbercoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sbercoind.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			sbercoind.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			sbercoind.unsubscribeAddressAll(emitter1);
			sbercoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			sbercoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			sbercoind.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(sbercoind.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var config = sbercoind._getDefaultConfig();
			config.should.equal(defaultSbercoinConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a sbercoin.conf file', function() {
			var TestSbercoin = proxyquire('../../lib/services/sbercoind', {
				fs: {
					readFileSync: readFileSync,
			...
			var sbercoind = new TestSbercoin(baseConfig);
			sbercoind.options.spawn.datadir = '/tmp/.sbercoin';

			sinon.spy(sbercoind, '_expandRelativeDatadir');
			sinon.spy(sbercoind, '_checkConfigIndexes');
			sinon.spy(sbercoind, '_getDefaultConf')
			sinon.spy(sbercoind, '_getNetworkConfigPath');
			sinon.spy(sbercoind, '_parseBitcoinConf');

			var node = {};
			sbercoind._loadSpawnConfiguration(node);

			sbercoind._expandRelativeDatadir.callCount.should.equal(1);
			sbercoind._checkConfigIndexes.callCount.should.equal(1);
			sbercoind._getDefaultConf.callCount.should.equal(1);
			sbercoind._getNetworkConfigPath.callCount.should.equal(1);
			sbercoind._parseBitcoinConf.callCount.should.equal(2);

			should.exist(sbercoind.spawn.config);
			sbercoind.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: sbercoincore.Networks.testnet,
					configPath: '/tmp/.sbercoincore/sbercoincore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			var sbercoind = new TestSbercoin(config);
			...
			sbercoind.options.spawn.datadir = './data';
			var node = {};
			sbercoind._loadSpawnConfiguration(node);
			...
			sbercoind.options.spawn.datadir.should.equal('/tmp/.sbercoincore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var TestSbercoin = proxyquire('../../lib/services/sbercoind', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badsbercoin.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var sbercoind = new TestSbercoin(baseConfig);
			(function() {
				sbercoind._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(sbercoincore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the sbercoin.conf file', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				sbercoind._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the sbercoin.conf file', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the sbercoin.conf file', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the sbercoin.conf file', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the sbercoin.conf file', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				sbercoind.transactionDetailedCache.set(keys[i], {});
				sbercoind.utxosCache.set(keys[i], {});
				sbercoind.accountInfo.set(keys[i], {});
				sbercoind.txidsCache.set(keys[i], {});
				sbercoind.balanceCache.set(keys[i], {});
				sbercoind.summaryCache.set(keys[i], {});
				sbercoind.blockOverviewCache.set(keys[i], {});
				sbercoind.blockJsonCache.set(keys[i], {});
			}
			sbercoind._resetCaches();
			should.equal(sbercoind.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(sbercoind.utxosCache.get(keys[0]), undefined);
			should.equal(sbercoind.accountInfo.get(keys[0]), undefined);
			should.equal(sbercoind.txidsCache.get(keys[0]), undefined);
			should.equal(sbercoind.balanceCache.get(keys[0]), undefined);
			should.equal(sbercoind.summaryCache.get(keys[0]), undefined);
			should.equal(sbercoind.blockOverviewCache.get(keys[0]), undefined);
			should.equal(sbercoind.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.tryAllInterval = 1;
			sbercoind.nodes.push({
				client: {
			...
			sbercoind._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				sbercoind.nodes[0].client.getInfo.callCount.should.equal(1);
				sbercoind.nodes[1].client.getInfo.callCount.should.equal(1);
				sbercoind.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			sbercoind._tryAllClients(function(client, next) {
				...
				sbercoind.nodes[1].client.getInfo.callCount.should.equal(1);
				sbercoind.nodes[2].client.getInfo.callCount.should.equal(1);
				sbercoind.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert sbercoind-rpc error object into JavaScript error', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var error = sbercoind._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			sbercoind.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			sbercoind.nodes.push({
				client: {
			...
			sbercoind._initChain(function() {
				log.info.callCount.should.equal(1);
				sbercoind.getRawBlock.callCount.should.equal(1);
				sbercoind.getRawBlock.args[0][0].should.equal('genesishash');
				sbercoind.height.should.equal(5000);
				sbercoind.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		it('it will handle error from getBestBlockHash', function(done) {
			...
			sbercoind._initChain(function(err) {
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			sbercoind.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			sbercoind._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			sbercoincore.Networks.disableRegtest();
			baseConfig.node.network = sbercoincore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: sbercoincore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var sbercoind = new SbercoinService(config);
			sbercoind._getDefaultConf().rpcport.should.equal(8332);
		});
		it('will get default rpc port for testnet', function() {
			...
			var sbercoind = new SbercoinService(config);
			sbercoind._getDefaultConf().rpcport.should.equal(18332);
		});
		it('will get default rpc port for regtest', function() {
			sbercoincore.Networks.enableRegtest();
			var config = {
				node: {
					network: sbercoincore.Networks.testnet
				},
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var sbercoind = new SbercoinService(config);
			should.equal(sbercoind._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var sbercoind = new SbercoinService(config);
			sbercoind._getNetworkConfigPath().should.equal('testnet3/sbercoin.conf');
		});
		it('will get default rpc port for regtest', function() {
			sbercoincore.Networks.enableRegtest();
			var config = {
			...
			var sbercoind = new SbercoinService(config);
			sbercoind._getNetworkConfigPath().should.equal('regtest/sbercoin.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.node.network = sbercoincore.Networks.testnet;
			sbercoind._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.node.network = sbercoincore.Networks.testnet;
			sbercoincore.Networks.enableRegtest();
			sbercoind._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.node.network = sbercoincore.Networks.livenet;
			sbercoincore.Networks.enableRegtest();
			should.equal(sbercoind._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			sbercoind._rapidProtectedUpdateTip = sinon.stub();
			sbercoind.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			sbercoind._zmqBlockHandler(node, message);
			sbercoind._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			var message = new Buffer('76843f1dfe455347d897e757ff5ff51a16a0f62b5d02a659c5680392de3a2b89', 'hex');
			sbercoind._rapidProtectedUpdateTip = sinon.stub();
			sinon.spy(sbercoind, 'emit');
			sbercoind.on('block', function(block) {
				block.should.equal(message);
				sbercoind.emit.callCount.should.equal(1);
				sbercoind._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			sbercoind._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			sbercoind._zmqBlockHandler(node, message);
			sbercoind._rapidProtectedUpdateTip.callCount.should.equal(1);
			sbercoind._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			sbercoind._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(sbercoind, 'emit');
			var emitter = new EventEmitter();
			sbercoind.subscriptions.hashblock.push(emitter);
			emitter.on('sbercoind/hashblock', function(blockHash) {
				sbercoind._rapidProtectedUpdateTip.callCount.should.equal(1);
				sbercoind.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				sbercoind._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.syncPercentage = sinon.stub();
			sinon.spy(sbercoind, '_resetCaches');
			sbercoind.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				sbercoind._resetCaches.callCount.should.equal(1);
			...
			sbercoind._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			sbercoind.on('synced', function() {
				sbercoind.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			sbercoind.on('synced', function() {
				sbercoind.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			sbercoind.on('error', function(err) {
				log.error.callCount.should.equal(1);
				sbercoind.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.syncPercentage = sinon.stub();
			sbercoind._resetCaches = sinon.stub();
			sbercoind.on('tip', function(height) {
				sbercoind._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				sbercoind.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using sbercoincore.Transaction', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = sbercoincore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(sbercoincore.Networks.testnet);
			var outputAddress = sbercoincore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = sbercoincore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: sbercoincore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = sbercoind._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var tx = sbercoincore.Transaction();
			tx.addInput(sbercoincore.Transaction.Input({
				prevTxId: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				script: sbercoincore.Script('OP_TRUE'),
				outputIndex: 1,
				output: {
					script: sbercoincore.Script('OP_TRUE'),
					satoshis: 5000000000
			...
			tx.addOutput(sbercoincore.Transaction.Output({
				script: sbercoincore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = sbercoind._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var tx = sbercoincore.Transaction();
			var address = sbercoincore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(sbercoincore.Transaction.Output({
				script: sbercoincore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(sbercoincore.Transaction.Output({
				script: sbercoincore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			sbercoind._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			sbercoind.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('sbercoind/addresstxid', function(data) {
			...
			sbercoind._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			sbercoind.subscriptions.rawtransaction.push(emitter);
			emitter.on('sbercoind/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			sbercoind._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(sbercoind, 'emit');
			sbercoind.subscriptions.rawtransaction.push(emitter);
			emitter.on('sbercoind/rawtransaction', function() {
				done();
			});
			var node = {};
			sbercoind._zmqTransactionHandler(node, expectedBuffer);
		...
		it('will emit "tx" event', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			var expectedBuffer = new Buffer(txhex, 'hex');
			sbercoind.on('tx', function(buffer) {
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind._updateTip = sinon.stub();
			sbercoind._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			sbercoind.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			sbercoind._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				sbercoind._updateTip.callCount.should.equal(11);
				sbercoind._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			sbercoind._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				sbercoind._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			sbercoind._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				sbercoind._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			sbercoind._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				sbercoind._zmqBlockHandler.callCount.should.equal(0);
				sbercoind._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var sbercoind = new SbercoinService(baseConfig);
			var node = {};
			sbercoind._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			sbercoind._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			sbercoind._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			sbercoind._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
		...
		it('will set height and emit tip', function(done) {
			...
			sbercoind.on('tip', function(height) {
				...
				sbercoind.height.should.equal(100);
				done();
			});
			sbercoind._loadTipFromNode(node, function(err) {
				if (err) {
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestSbercoinService = proxyquire('../../lib/services/sbercoind', {
				fs: {
					readFile: readFile
				}
			});
			var sbercoind = new TestSbercoinService(baseConfig);
			sbercoind.spawnStopTime = 1;
			sbercoind._process = {};
			sbercoind._process.kill = sinon.stub();
			sbercoind._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				sbercoind._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			sbercoind._process.kill = sinon.stub().throws(error2);
			sbercoind._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind._loadSpawnConfiguration = sinon.stub();
			sbercoind._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			sbercoind._spawnChildProcess(function(err) {
				sbercoind._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind._loadSpawnConfiguration = sinon.stub();
			sbercoind._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			sbercoind._spawnChildProcess(function(err) {
				sbercoind._loadSpawnConfiguration.callCount.should.equal(1);
				sbercoind._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var sbercoind = new TestSbercoinService(config);
			sbercoind.spawn = {};
			sbercoind._loadSpawnConfiguration = sinon.stub();
			sbercoind._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			sbercoind.node.stopping = true;
			sbercoind._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			sbercoind._loadSpawnConfiguration = sinon.stub();
			sbercoind.spawn = {};
			sbercoind.spawn.exec = 'testexec';
			sbercoind.spawn.configPath = 'testdir/sbercoin.conf';
			sbercoind.spawn.datadir = 'testdir';
			sbercoind.spawn.config = {};
			sbercoind.spawn.config.rpcport = 20001;
			sbercoind.spawn.config.rpcuser = 'sbercoin';
			sbercoind.spawn.config.rpcpassword = 'password';
			sbercoind.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			sbercoind._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			sbercoind._initZmqSubSocket = sinon.stub();
			sbercoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			sbercoind._checkReindex = sinon.stub().callsArgWith(1, null);
			sbercoind._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/sbercoin.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				sbercoind._loadTipFromNode.callCount.should.equal(1);
				sbercoind._initZmqSubSocket.callCount.should.equal(1);
				should.exist(sbercoind._initZmqSubSocket.args[0][0].client);
				sbercoind._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				sbercoind._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(sbercoind._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn sbercoind spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestSbercoinService = proxyquire('../../lib/services/sbercoind', {
				fs: {
			...
			sbercoind._loadSpawnConfiguration = sinon.stub();
			sbercoind.spawn = {};
			sbercoind.spawn.exec = 'sbercoind';
			sbercoind.spawn.datadir = '/tmp/sbercoin';
			sbercoind.spawn.configPath = '/tmp/sbercoin/sbercoin.conf';
			sbercoind.spawn.config = {};
			sbercoind.spawnRestartTime = 1;
			sbercoind._loadTipFromNode = sinon.stub().callsArg(1);
			sbercoind._initZmqSubSocket = sinon.stub();
			sbercoind._checkReindex = sinon.stub().callsArg(1);
			sbercoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			sbercoind._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(sbercoind, '_spawnChildProcess');
			sbercoind._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						sbercoind._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			sbercoind._spawnChildProcess(function(err) {
				...
				sbercoind._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				sbercoind.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn sbercoind spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestSbercoinService = proxyquire('../../lib/services/sbercoind', {
				fs: {
			...
			sbercoind._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						sbercoind._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			var sbercoind = new TestSbercoinService(baseConfig);
			sbercoind.startRetryInterval = 1;
			sbercoind._loadSpawnConfiguration = sinon.stub();
			sbercoind.spawn = {};
			sbercoind.spawn.exec = 'testexec';
			sbercoind.spawn.configPath = 'testdir/sbercoin.conf';
			...
			sbercoind.spawn.config.rpcpassword = 'password';
			sbercoind.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			sbercoind._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			sbercoind._spawnChildProcess(function(err) {
				sbercoind._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			sbercoind._initZmqSubSocket = sinon.stub();
			sbercoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			sbercoind._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var sbercoind = new SbercoinService(config);
			sbercoind.node.stopping = true;
			sbercoind.startRetryInterval = 100;
			sbercoind._loadTipFromNode = sinon.stub();
			sbercoind._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to sbercoind/);
				sbercoind._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			sbercoind.startRetryInterval = 1;
			var config = {};
			sbercoind._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				sbercoind._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			sbercoind._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			sbercoind._connectProcess(config, function(err, node) {
				should.not.exist(err);
				sbercoind._loadTipFromNode.callCount.should.equal(1);
				sbercoind._initZmqSubSocket.callCount.should.equal(1);
				sbercoind._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.options = {};
			sbercoind.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('Sbercoin configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			sbercoind.options = {
				connect: [
					{}
				]
			...
			sbercoind.start(function(err) {
				sbercoind._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			var node = {};
			sbercoind._initChain = sinon.stub().callsArg(0);
			sbercoind._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			sbercoind.options = {
				spawn: {}
			};
			sbercoind.start(function(err) {
				should.not.exist(err);
				sbercoind.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(sbercoind, '_spawnChildProcess');
			var nodes = [{}];
			sbercoind._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			sbercoind.options = {
				connect: [
					{}
				]
			...
			sbercoind.start(function(err) {
				should.not.exist(err);
				sbercoind._initChain.callCount.should.equal(1);
				sbercoind._connectProcess.callCount.should.equal(1);
				sbercoind.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		it('will give error from syncPercentage', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			sbercoind.isSynced(function(err) {
				should.exist(err);
				sbercoind.syncPercentage.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			sbercoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				sbercoind.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			sbercoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			sbercoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			sbercoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.syncPercentage(function(err) {
				should.exist(err);
				sbercoind.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			sbercoind.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				sbercoind.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var args = sbercoind._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var args = sbercoind._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sinon.spy(sbercoind, '_normalizeAddressArg');
			sbercoind.nodes.push({
				client: {
			...
			var options = {};
			sbercoind.getAddressBalance(address, options, function(err) {
				sbercoind._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			sbercoind.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				sbercoind.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					sbercoind._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			sbercoind.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = sbercoind._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = sbercoind._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = sbercoind._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = sbercoind._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				sbercoind._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			sbercoind.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(sbercoind, '_getHeightRangeQuery');
			sinon.spy(sbercoind, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			sbercoind.getAddressTxids(address, options, function(err) {
				should.exist(err);
				sbercoind._getHeightRangeQuery.callCount.should.equal(1);
				sbercoind._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			sbercoind.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				sbercoind._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			sbercoind.getAddressTxids(address, options, function(err, txids) {
				...
				sbercoind.getAddressTxids(address, options, function(err, txids) {
					...
					sbercoind._getHeightRangeQuery.callCount.should.equal(2);
					sbercoind._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			sbercoind.getAddressTxids(address, options, function(err, txids) {
				...
				sbercoind.getAddressTxids(address, options, function(err, txids) {
					...
					sbercoind._getHeightRangeQuery.callCount.should.equal(4);
					sbercoind._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			sbercoind.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				sbercoind.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					sbercoind.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						sbercoind._getHeightRangeQuery.callCount.should.equal(3);
						sbercoind._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.height = 10;
			var confirmations = sbercoind._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var result = {};
			sbercoind._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var result = {};
			sbercoind._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var result = {};
			sbercoind._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var result = {};
			sbercoind._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(sbercoind, '_getAddressDetailsForInput');
			sinon.spy(sbercoind, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = sbercoind._getAddressDetailsForTransaction(tx, addresses);
			...
			sbercoind._getAddressDetailsForInput.callCount.should.equal(1);
			sbercoind._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var sbercoind = new SbercoinService(baseConfig);
			sinon.spy(sbercoind, '_getConfirmationsDetail');
			sbercoind.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			sbercoind.height = 300;
			var addresses = {};
			sbercoind._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			sbercoind._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				sbercoind.getDetailedTransaction.callCount.should.equal(1);
				sbercoind._getAddressDetailsForTransaction.callCount.should.equal(1);
				sbercoind._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			sbercoind._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				sbercoind.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from sbercoincore addresses', function() {
			var addresses = [
				sbercoincore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				sbercoincore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var sbercoind = new SbercoinService(baseConfig);
			var strings = sbercoind._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var sbercoind = new SbercoinService(baseConfig);
			(function() {
				sbercoind._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var sbercoind = new SbercoinService(baseConfig);
			sinon.spy(sbercoind, '_normalizeAddressArg');
			sbercoind.maxAddressesQuery = 100;
			sbercoind.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				sbercoind._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sinon.spy(sbercoind, '_normalizeAddressArg');
			sinon.spy(sbercoind, '_getAddressStrings');
			sbercoind.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				sbercoind._normalizeAddressArg.callCount.should.equal(1);
				sbercoind._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(sbercoind, '_normalizeAddressArg');
			sinon.spy(sbercoind, '_getAddressStrings');
			sbercoind.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			sbercoind.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				sbercoind._normalizeAddressArg.callCount.should.equal(1);
				sbercoind._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(sbercoind, '_normalizeAddressArg');
			sinon.spy(sbercoind, '_getAddressStrings');
			sbercoind.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			sbercoind._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			sbercoind.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				sbercoind._normalizeAddressArg.callCount.should.equal(1);
				sbercoind._getAddressStrings.callCount.should.equal(1);
				sbercoind.getAddressTxids.callCount.should.equal(1);
				sbercoind._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(sbercoind, '_normalizeAddressArg');
			sinon.spy(sbercoind, '_getAddressStrings');
			sinon.spy(sbercoind, 'getAddressTxids');
			sbercoind.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			sbercoind.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				sbercoind._normalizeAddressArg.callCount.should.equal(1);
				sbercoind._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(sbercoind, '_normalizeAddressArg');
			sinon.spy(sbercoind, '_getAddressStrings');
			sinon.spy(sbercoind, '_paginateTxids');
			sbercoind._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(sbercoind, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			sbercoind.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			sbercoind.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				sbercoind._normalizeAddressArg.callCount.should.equal(1);
				sbercoind._getAddressStrings.callCount.should.equal(1);
				sbercoind.getAddressTxids.callCount.should.equal(1);
				sbercoind._getAddressDetailedTransaction.callCount.should.equal(2);
				sbercoind._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			sbercoind.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			sbercoind.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			sbercoind.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			sbercoind.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			sbercoind.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			sbercoind.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			sbercoind.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			sbercoind.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			sbercoind.getAddressSummary(address, options, function(err, summary) {
				sbercoind._paginateTxids.callCount.should.equal(1);
				sbercoind._normalizeAddressArg.callCount.should.equal(1);
				sbercoind.getAddressTxids.callCount.should.equal(1);
				sbercoind.getAddressBalance.callCount.should.equal(1);
				sbercoind._paginateTxids.args[0][1].should.equal(0);
				sbercoind._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			sbercoind.getAddressSummary(address, options, function(err, summary) {
				...
				sbercoind.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					sbercoind._paginateTxids.callCount.should.equal(0);
					sbercoind._normalizeAddressArg.callCount.should.equal(2);
					sbercoind.getAddressTxids.callCount.should.equal(1);
					sbercoind.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			sbercoind.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				sbercoind._paginateTxids.callCount.should.equal(1);
				sbercoind._normalizeAddressArg.callCount.should.equal(1);
				sbercoind.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			sbercoind._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			sbercoind.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(sbercoind, '_tryAllClients');
			sbercoind.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			sbercoind.getRawBlock(blockhash, function(err) {
				should.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			sbercoind.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			sbercoind.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			sbercoind.getBlock(0, function(err) {
				sbercoind._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as sbercoincore object from height', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sinon.spy(sbercoind, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			sbercoind.getBlock(0, function(err, block) {
				should.not.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(sbercoincore.Block);
				done();
			});
		});
		it('will getblock as sbercoincore object', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sinon.spy(sbercoind, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			sbercoind.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			sbercoind.getBlock(hash, function(err, block) {
				should.not.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			sbercoind.getBlock(0, function(err, block) {
				...
				sbercoind.getBlock(0, function(err, block) {
					should.not.exist(err);
					sbercoind._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			sbercoind.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var sbercoind = new SbercoinService(baseConfig);
			sinon.spy(sbercoind, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			sbercoind.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			sbercoind.getBlockHeader(10, function(err) {
				sbercoind._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			sbercoind.getBlockHeader(blockhash, function(err) {
				sbercoind._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			sbercoind.getBlockHeader(0, function(err) {
				sbercoind._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			sbercoind.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				sbercoind._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			sbercoind.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				sbercoind._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			sbercoind._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				sbercoind._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			sbercoind._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			sbercoind._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sinon.spy(sbercoind, '_maybeGetBlockHash');
			sbercoind._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			sbercoind.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				sbercoind._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			sbercoind.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				sbercoind.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					sbercoind._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			sbercoind.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = sbercoincore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			sbercoind.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			sbercoind.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			sbercoind.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = sbercoincore.Transaction();
			(function() {
				sbercoind.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.getRawTransaction('txid', function(err) {
				should.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			sbercoind.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.getTransaction('txid', function(err) {
				should.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			sbercoind.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				sbercoind._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(sbercoincore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			sbercoind.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				sbercoind._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			sbercoind.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			sbercoind.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			sbercoind.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.getInfo(function(err) {
				should.exist(err);
				err.should.be.an.instanceof(errors.RPCError);
				done();
			});
		...
		it('will call client getInfo and give result', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.node.getNetworkName = sinon.stub().returns('testnet');
			...
			sbercoind.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			sbercoind.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			sbercoind.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				sbercoind.getSubsidy.callCount.should.equal(1);
				sbercoind.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			sbercoind.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.spawn = {};
			sbercoind.spawn.process = new EventEmitter();
			sbercoind.spawn.process.kill = sinon.stub();
			sbercoind.stop(done);
			sbercoind.spawn.process.kill.callCount.should.equal(1);
			sbercoind.spawn.process.kill.args[0][0].should.equal('SIGINT');
			sbercoind.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			sbercoind.spawn.process.kill.callCount.should.equal(1);
			sbercoind.spawn.process.kill.args[0][0].should.equal('SIGINT');
			sbercoind.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var sbercoind = new SbercoinService(baseConfig);
			sbercoind.shutdownTimeout = 300;
			sbercoind.spawn = {};
			sbercoind.spawn.process = new EventEmitter();
			sbercoind.spawn.process.kill = sinon.stub();
			sbercoind.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				sbercoind._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var sbercoind = new SbercoinService(baseConfig);
			sinon.spy(sbercoind, '_normalizeAddressArg');
			sinon.spy(sbercoind, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			sbercoind.nodes.push({
			...
			sbercoind.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				sbercoind._getBalanceFromMempool.callCount.should.equal(1);
				sbercoind._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			sbercoind.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			sbercoind.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			sbercoind.getJsonBlock(0, function(err) {
				should.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			sbercoind.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			sbercoind.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(2);
				sbercoind._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			sbercoind.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(1);
				sbercoind._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var emitter = new EventEmitter();
			sbercoind.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(sbercoind.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var sbercoind = new SbercoinService(baseConfig);
			var emitter = new EventEmitter();
			sbercoind.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			sbercoind.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			sbercoind.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			sbercoind.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			sbercoind.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			sbercoind.unsubscribeBalance(emitter1);
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			sbercoind.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			sbercoind.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var sbercoind = new SbercoinService(baseConfig);
			...
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			sbercoind.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			sbercoind.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var sbercoind = new SbercoinService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			sbercoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(sbercoind, 'unsubscribeBalanceAll');
			sbercoind.unsubscribeBalance(emitter1);
			sbercoind.unsubscribeBalanceAll.callCount.should.equal(1);
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			sbercoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			sbercoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sbercoind.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			sbercoind.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			sbercoind.unsubscribeBalanceAll(emitter1);
			sbercoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			sbercoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			sbercoind.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(sbercoind.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var sbercoind = new SbercoinService(baseConfig);
			sinon.spy(sbercoind, '_notifyBalanceSubscriber');
			sbercoind._getAddressesFromTransaction = sinon.stub().returns(addresses);
			sbercoind.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				sbercoind._notifyBalanceSubscriber.callCount.should.equal(1);
				sbercoind._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			sbercoind._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				sbercoind.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			sbercoind._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			sbercoind.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.getDgpInfo(function(err, result) {
				should.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.getMiningInfo(function(err, result) {
				should.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			sbercoind.getStakingInfo(function(err, result) {
				should.exist(err);
				sbercoind._tryAllClients.callCount.should.equal(1);
